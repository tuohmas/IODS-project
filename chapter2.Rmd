...
This week, we are working with learning2014 dataset. For a detailed descrption, [see the data code book](https://www.mv.helsinki.fi/home/kvehkala/JYTmooc/JYTOPKYS2-meta.txt) (in Finnish)
...
Let us begin by reading learning data as data.frame from local directory:

```{r}
lrn14 <- read.csv("~/R/IODS-project/data/learning2014.csv")
...
1. attitude (attitude), Pearson correlation. 0.437;
2. strategic learning (stra), corr: 0.146; and
3. surface learning (surf), corr: -0.144
...
From the summary we see that. From explanatory variables only _attitude_ passes
null-hypothesis test with alpha-level 0.5 ($p < 1.93×10^{-8}$), whereas 
...
* $\alpha$ is a constant representig the point where regression line intersects the y axis, and 
...
* Estimated effect of attitude is positive ($\beta=0.352$). From associated _F_-test we can determine that the beta coefficient is significantly different from zero ($p < 4.12×10^{-9}$), pointing a statistically significant relationship between $y$ and $x$.
...
For this end, lets consider three diagnostic plots:
...
Finally, let's produce a diagnostic plots using the plot() function: Residuals vs Fitted values, Normal QQ-plot and Residuals vs Leverage using the argument which (by passing it a vector c(1, 2, 5) corresponding to the above graphics). 
